...
void schimbareNumarLocuinta(struct Locuinta** matrice, int nr_linii, int* nr_coloane, int numarLocuinta) {
	...
	if (idLocuinta != -1) {
		...
		inserareLocuinta(&(matrice[indexMatrice]), &(nr_coloane[indexMatrice]), matrice[pozitie][indexInVector]); //functia face shallow copy
		stergeLocuinta(&(matrice[pozitie]), &(nr_coloane[pozitie]), idLocuinta); //functia nu sterge complet
	}
}
